...
class VisitManagerTest extends WebTestCase
{
    /**
     * Test
    ...
    public function testCalculPrice()
    {
        ...
        $price = $visitManager->calculPrice($visit);
     ...
     * @throws \Exception
     */
    public function testCalculTicketPrice()
    {
        $visitManager = $this->VisitManagerAndDependenciesMocks();
        $visit = $this->createOneVisitForSenior();
        $price = $visitManager->calculTicketPrice($visit->getTickets()->first());
    ...
    public function VisitManagerAndDependenciesMocks()
    {
        ...
        $visitManager = new VisitManager($session);
        return $visitManager;
    }
    ...
    {
        $visit = new Visit;
        $date = new \DateTime("27-05-2020");
        $visit->setVisitDate($date);
        $type = Visit::TYPE_FULL_DAY;
        $visit->setType($type);
        $visit->addTicket($this->createTicket("11-11-1957", 1));
        return $visit;
    }
    ...
    public function createTicket($date, $discount)
    {
        ...
        $ticket->setReducedPrice($discount);
        return $ticket;
    }
